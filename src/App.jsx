...
const App = () => {
  ...
  const getWeather = async () => {
    ...
    toast.info(`Fetching weather report for ${capitalizeFirstLetter(cityName)} `)
    await getFormattedWeatherData({...query, units}).then((data) => {
      toast.success(`Fetched weather report for ${data.name}, ${data.country}`)
      setWeather(data);
    });
  }
  ...
  return (
    <div className={`mx-auto max-w-screen-lg mt-4 py-5 px-32 bg-gradient-to-br shadow-xl shadow-gray-400 ${formatBackground ()}`}>
      ...
      {weather ? (
        ...
        <div className="text-2xl flex justify-center">{"Loading weather data..."}</div>
      )}
      {/* {weather && (
        <>
          <TimeAndLocation weather={weather}/>
          <TempAndDetails weather={weather} units={units}/>
          <Forecast title="3hour step forecast" data={weather.hourly} />
          <Forecast title="daily step forecast" data={weather.daily} />        
        </>
      )} */}
